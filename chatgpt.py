...
def extract_category_hierarchy(page_text: str):
    """
    Extract category / subcategory / childcategory from breadcrumb:
    Home / Products / <category> / <subcategory> / <childcategory>
    Returns (category, subcategory, childcategory) or ("","","") if not found.
    """
    category = subcategory = childcategory = ""
    m = re.search(
        r"Home\s*/\s*Products\s*/\s*([^/]+?)\s*/\s*([^/]+?)\s*/\s*([^/]+?)\s",
        page_text
    )
    if m:
        category = m.group(1).strip()
        subcategory = m.group(2).strip()
        childcategory = m.group(3).strip()
...
def extract_products_from_text(page_text: str, page_num: int = 0) -> List[Dict[str, str]]:
    ...
    for match in matches:
        ...
        # Prefer the description from the product detail page
        detail_desc = fetch_description_from_detail(lcsc_code.strip())
        if detail_desc:
            description = detail_desc
        else:
            # Fallback to whatever we can infer from the category page
...
def scrape_lcsc_category(base_url: str, max_pages: int) -> pd.DataFrame:
    ...
    # Will be filled from breadcrumb on first successful page
    category_name = ""
    subcategory_name = ""
    childcategory_name = ""
    ...
    for page in range(1, max_pages + 1):
        ...
        # On first page, try to parse category hierarchy
        if not category_name:
            cat, subcat, child = extract_category_hierarchy(text)
            category_name = cat or ""
            subcategory_name = subcat or ""
            childcategory_name = child or ""
        ...
        if len(products) == 0:
            print(f"    [*] Page {page}: 0 matches; assuming no more pages.")
            break

        # Filter out duplicates and attach category info
        new_count = 0
        for p in products:
            ...
            p["childcategory"] = childcategory_name
            all_rows.append(p)
            new_count += 1
...
def main():
    print("=" * 80)
    print("LCSC WEB SCRAPER - WITH DESCRIPTION + CATEGORY EXTRACTION")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("TIPS:")
    print("  - If descriptions or category fields are missing, set DEBUG_MODE = True and re-run")
    print("  - Check lcsc_page1_text.txt to see the actual page text format")
    print("  - Adjust the regex patterns if needed based on the text format")
    print("=" * 80)
